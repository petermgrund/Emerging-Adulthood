...
```{r Select variables}
df_analysis <- df %>%
  select(ID, mindful_1:mindful_biascheck, sex:race, school_coded, IDEA_1:`IDEA-bias-dummy`, income, stress_1:stress_bias_dummy, `moa1#1_1`:`MOA-ACH_dummy`) %>%
...
# Linear model 1: Regressing achievement score onto stress
...
```{r}
reg_achieve_stress <- ggplot(df_analysis, aes(x = achievement, y = stress_score)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "green") 
reg_achieve_stress
```

## Graphic of regression of stress onto mindfulness
```{r}
reg_mindf_stress <- ggplot(df_analysis, aes(x = mindful_score, y = stress_score)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "green") +
